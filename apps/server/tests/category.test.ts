...
import { createDummyCategory, deleteDummyCategory } from './utils/dummy-categories'
...
describe('Category routes test', () => {
  ...
  describe('Delete category - DELETE/:id', async () => {
    it('should post and delete category', async () => {
      ...
      const { data: category, status: newStatus } = await api.categories({ id: oldCategory!.id }).delete()
