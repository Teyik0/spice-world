			...
			test("admin can update order status", async () => {
					...
					data: {
						...
						variants: {
							create: {
								...
								polarProductId: `mock_polar_${testId}`,
							},
						},
					},
					include: {
				...
				const order = await testDb.client.order.create({
					data: {
						...
						status: "PENDING",
						polarCheckoutId: `checkout_${testId}`,
			...
			test("admin can view all orders", async () => {
				...
				await testDb.client.order.create({
					data: {
						userId: normalUser.user.id,
						status: "PENDING",
						polarCheckoutId: `checkout_${testId}_1`,
						subtotalAmount: 10.99,
						shippingAmount: 5,
						totalAmount: 15.99,
						shippingAddress: {
				...
				await testDb.client.order.create({
					data: {
						...
						status: "PAID",
						polarCheckoutId: `checkout_${testId}_2`,
						subtotalAmount: 10.99,
						shippingAmount: 5,
						totalAmount: 15.99,
						shippingAddress: {
