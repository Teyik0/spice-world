...
// import type { User, AdditionalUserFieldsInput, Session} from 'better-auth'
// import type { User } from '../src/prisma/client'
...
describe('BetterAuth Plugin Tests', () => {
  ...
  beforeAll(async () => {
    ...
    normalUser = {
      user: { ...user, id: 'normal-user', name: 'Normal User', email: 'normal@example.com', role: 'user' },
      session: {
        ...session,
        id: 'normal-session',
        userId: 'normal-user',
  ...
  describe('Email Sending', () => {
    it('should send reset password email correctly', async () => {
      const sendEmailMock = spyOn(auth.options.emailAndPassword, 'sendResetPassword')
    ...
    it('should send change email verification correctly', async () => {
      const sendEmailMock = spyOn(auth.options.user.changeEmail, 'sendChangeEmailVerification')
    ...
    it('should send email verification correctly', async () => {
      const sendEmailMock = spyOn(auth.options.emailVerification, 'sendVerificationEmail')
  ...
  describe('Social Providers', () => {
    it('should configure Google social provider correctly', () => {
      ...
      expect(auth.options.socialProviders.google.clientId).toBe(process.env.GOOGLE_CLIENT_ID as string)
      expect(auth.options.socialProviders.google.clientSecret).toBe(process.env.GOOGLE_CLIENT_SECRET as string)
    })
  })
})
