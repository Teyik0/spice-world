...
describe('Product routes test', () => {
  ...
  describe('GET /products/count', () => {
    ...
    it('should return the count of products filtered by status', async () => {
      ...
      expect(data).toBe(testProducts.filter((p) => p.status === 'PUBLISHED').length)
    })
  })
  ...
  describe('GET /products/:id', () => {
    it('should return a product by id', async () => {
      const { data, status } = await api.products({ id: testProducts[0].id }).get()
  ...
  describe('PATCH /products/:id', () => {
    ...
    it('should update the product successfully', async () => {
      ...
      const { data, status } = await api.products({ id: testProducts[0].id }).patch({
      ...
      expect(productData.variants.length).toBe(updatedProductData.variants.length)
      expect(productData.variants[0].price).toBe(updatedProductData.variants[0].price)
      expect(productData.variants[0].stock).toBe(updatedProductData.variants[0].stock)
      expect(productData.images.length).toBe(testProducts[0].images.length + updatedProductData.images.length)
    ...
    it('should return an error update name conflict with an existing product', async () => {
      ...
      const { status, error } = await api.products({ id: testProducts[0].id }).patch(updatedProductData)
  ...
  describe('DELETE /products/:id', () => {
    it('should delete a product successfully', async () => {
      ...
      const { data, status } = await api.products({ id: productToDelete.id }).delete()
      ...
      const { data: deletedProduct, status: getStatus } = await api.products({ id: productToDelete.id }).get()
    ...
    it('should return an error if the product ID does not exist', async () => {
      ...
      const { status, error } = await api.products({ id: nonExistentProductId }).delete()
  ...
  describe('POST /products/:id/images', () => {
    it('should upload new images successfully', async () => {
      ...
      const { data, status } = await api.products({ id: productToUpdate.id }).images.post({
        images: [Bun.file(filePath1), Bun.file(filePath2)],
      })
    ...
    it('should return an error if the maximum number of images is exceeded', async () => {
      ...
      const { error, status } = await api.products({ id: productToUpdate.id }).images.post({
        images: [Bun.file(filePath1), Bun.file(filePath2), Bun.file(filePath3)],
      })
