...
import { FormError, formAction$, useForm, valiForm$, type InitialValues } from '@modular-forms/qwik'
...
export const useSignInFormLoader = routeLoader$<InitialValues<SignInForm>>(({ cookie }) => {
  return {
...
export const useFormSignInAction = formAction$<SignInForm>(async (values, { cookie, redirect }) => {
  ...
  return (
    <Card.Root class="max-w-md rounded-b-lg rounded-l-lg">
      <Card.Header>
        ...
        <Card.Description class="text-xs md:text-sm">Enter your email below to login to your account</Card.Description>
      </Card.Header>
        ...
        <div class="grid gap-4">
          <Form class="grid gap-4">
            ...
            <div class="flex items-center">
              <Link href="#" class="ml-auto inline-block text-sm underline">
              ...
              <Field name="rememberme" type="boolean">
                {(field, props) => (
                  <div>
                    ...
                    {field.error && <div class="text-xs text-red-700">{field.error}</div>}
                  </div>
                )}
              </Field>
            ...
            <Button type="submit" class="w-full" disabled={loginForm.submitting}>
              {loginForm.submitting ? <LuLoader2 class="animate-spin h-6 w-6" /> : 'Login'}
            </Button>
          </Form>
          ...
          <div class="w-full gap-2 flex items-center justify-between flex-col">
            ...
            >
              ...
              {googleLoading.value ? <LuLoader2 class="animate-spin h-6 w-6" /> : 'Sign in with Google'}
            </Button>
          </div>
        </div>
      </Card.Content>
